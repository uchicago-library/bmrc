...
def forwards(apps, schema_editor):
    StandardPage = apps.get_model("standard", "StandardPage")
    _migrate_page_body(StandardPage, FROM_BLOCK, TO_BLOCK)


def backwards(apps, schema_editor):
    StandardPage = apps.get_model("standard", "StandardPage")
    _migrate_page_body(StandardPage, TO_BLOCK, FROM_BLOCK)


class Migration(migrations.Migration):

    dependencies = [
        ("standard", "0016_alter_standardpage_body"),
    ]
